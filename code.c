...
void displayModeMenu() {
  ...
  int startMode = max(1, min(9, menuPosition - 1));
  int endMode = min(9, startMode + 2);
  
  for (int i = startMode; i <= endMode; i++) {
    display.setCursor(10, (i - startMode + 2) * 12);
    if (i == menuPosition) {
      display.print("> ");
    } else {
      display.print("  ");
    }
    display.printf("Mode %d", i);
  }
}

void setModeFromMenu(int mode) {
  if (mode >= 1 && mode <= 9) {
    State newState = (State)(mode - 1);
...
void loop() {
  ...
  if (buttonPressed) {
    ...
    if ((millis() - lastDebounceTime) > debounceDelay) {
      ...
      if (!menuActive) {
        // Вход в меню
        menuActive = true;
        menuPosition = currentState + 1;
        selectedMode = menuPosition;
        displayModeMenu();
        Serial.println("Меню активировано");
      } else {
        // Выход из меню и установка режима
        menuActive = false;
        setModeFromMenu(selectedMode);
        Serial.printf("Режим %d установлен\n", selectedMode);
      }
      }
    }
  // ОБРАБОТКА ЭНКОДЕРА
  if (menuActive) {
    ...
    if (encoderMove != 0) {
      menuPosition += encoderMove;
      menuPosition = max(1, min(9, menuPosition));
      selectedMode = menuPosition;
      displayModeMenu();
